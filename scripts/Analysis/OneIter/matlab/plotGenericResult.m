        ...
        for i=1:size(scenarioType,2)
            for j=1:numOfMobileDevices
                try
                    ...
                    filePath = strcat(folderPath,'\NONMOVING',int2str(s),'\SIMRESULT_',char(scenarioType(i)),'_NEXT_FIT_',int2str(mobileDeviceNumber),'DEVICES_',appType,'_GENERIC.log')
                    
                    readData = dlmread(filePath,';',rowOfset,0);
                    value = readData(1,columnOfset);
                    %disp (value*readData(1,1))
                    if(calculatePercentage==1)
                		totalTask = readData(1,1)+readData(1,2);
                        value = (100 * value) / totalTask;
                    end
                    %Shaik addeed - remove line later - value = value*readData(1,1)
                    %value = value*readData(1,1)
    ...
    if(getConfiguration(20) == 1)
        for i=stepOfxAxis:stepOfxAxis:numOfMobileDevices
            ...
            for j=1:size(scenarioType,2)
                ...
                %shaik modified from -- results(j,i)
                hold on;
            end
        end
        ...
        for j=1:size(scenarioType,2)
            if(getConfiguration(19) == 1)
                ...
                %--Shaik changed orig is -- plot(types, results(j,:),':k','color',getConfiguration(20+j),'LineWidth',1.5);
